...
#     count_df = pd.DataFrame(index=range(1, 2**bitrate+1))
#     count_df['counts'] = 0
...
def check_if_seasonal(df, date_column, value_column):
    ...
    # Extract months only from non-null values
    months_present = df.loc[df[value_column].notna(), date_column].dt.month.unique()
    
    # Check if all months 1 to 12 are present
    return len(months_present) < 12
...
# def train_test_split_by_official_id(stations, nfolds):
#     """
...
# def compute_runoff_mean_and_SD(row):
#     """
...
#     monthly_data = df.groupby(['year', 'month'], group_keys=False)[stn_id].agg(['count', 'mean', 'stdev'])
#     days_in_month = df.groupby(['year', 'month'], group_keys=False).size()
...
#     extended_overlap_matrix = np.zeros((len(observations), n_bins))
...
#     # the "simulated" series is the proxy/donor series
#     # and the "observed" series is the target location
#     obs_count_df = df.groupby(target.obs_quantized_label, group_keys=False).count()
#     sim_count_df = df.groupby(target.sim_quantized_label, group_keys=False).count()
